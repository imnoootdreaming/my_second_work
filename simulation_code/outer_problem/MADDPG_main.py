...
def main():
    ...
    log_dir = f"runs/MADDPG/{today_str}_MADDPG_experiment"
    ...
    maddpg = MADDPG(env, agent_num, device, actor_lr, critic_lr, hidden_dim,
                    state_dims, action_dims, action_highs, action_lows, critic_input_dim, gamma, tau)
        ...
        for i_episode in range(num_episodes):
            ...
            for t in range(total_time_slots):
                if replay_buffer.size() < minimal_size:
                    ...
                    # 使用策略网络输出动作
                    actions = maddpg.take_action(s_norm, explore=True)
                ...
                if replay_buffer.size() >= minimal_size:
                    ...
                    # 处理 done：done_list 是长度 batch_size 的 list，每项是 int/float
    ...
    plt.title("MADDPG Training Performance")
    plt.show()
    ...
    filename = f"{today_str}_MADDPG_training_rewards_seed{seed}.csv"
