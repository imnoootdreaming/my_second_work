...
class MyEnv(gym.Env):
    def __init__(self, K, noma_group_num, uav_num, users_num_per_noma_group,
        ...
        self.safe_distance = 5  # UAV 之间的安全距离为 1m
        # UAV & 用户初始位置
        ...
        # 初始化奖励函数 和 线程池
        self.Reward = my_class_new.Reward(self.K, self.ris_pos, self.uav_num, self.x_max, self.y_max, self.safe_distance)
        ...
        self.action_space["uav"] = {
            f"uav_{i}": spaces.Box(
                low=np.array([self.epsilon, self.epsilon, self.epsilon, self.epsilon]),  # [角度, 距离, 卸载频率1, 卸载频率2]
                high=np.array([2 * np.pi, self.tau * self.speed_uav_max, 3e9, 3e9]),
                dtype=np.float32
            )
        ...
        self.action_space["ris"] = spaces.Box(
            low=np.concatenate([
                np.zeros(self.K),  # RIS 相位
                np.zeros(self.uav_num),  # first_trans_period
                np.zeros(self.uav_num),  # second_trans_period
                np.zeros(self.uav_num),  # 功率 P_k1_D1 最小 0
                np.zeros(self.uav_num),  # 功率 P_k2_D1 最小 0
                np.zeros(self.uav_num),  # 功率 P_k2_D2 最小 0
            ...
            high=np.concatenate([
                ...
                np.zeros(self.uav_num) + self.tau,  # second_trans_period
                np.zeros(self.uav_num) + self.P_max,  # 功率 P_k1_D1
                np.zeros(self.uav_num) + self.P_max,  # 功率 P_k2_D1
                np.zeros(self.uav_num) + self.P_max,  # 功率 P_k2_D2
    ...
    def step(self, actions, outer_iter):
        ...
        # 每个 NOMA 组的第二个调度时隙
        second_trans_period = ris_action[self.K + self.noma_group_num:self.K + self.noma_group_num * 2]
        # 每个 NOMA 组第一阶段第一个用户功率
        p_k1_D1 = ris_action[self.K + self.noma_group_num * 2:self.K + self.noma_group_num * 3]
        # 每个 NOMA 组第一阶段第二个用户功率
        p_k2_D1 = ris_action[self.K + self.noma_group_num * 3:self.K + self.noma_group_num * 4]
        # 每个 NOMA 组第二阶段第二个用户功率(转换后)
        p_k2_D2 = ris_action[self.K + self.noma_group_num * 4:self.K + self.noma_group_num * 5]
        # 每个用户选择的 UAV (长度 = num_users)
        # ------------------ 用户选择 ------------------
        uav_selection = np.ceil(ris_action[self.K + self.noma_group_num * 5:]).astype(int) - 1
        # print(f"============= UAV 选择系数为: {uav_selection} =============")
        uav_selection = np.clip(uav_selection, 0, self.uav_num - 1)  # DRL 优化选择 UAV
        # uav_selection = np.floor(ris_action[self.K + self.noma_group_num:]).astype(int)
        # uav_selection = np.clip(uav_selection, 0, self.uav_num - 1)  # 随机选择 UAV
        # ------------------ 用户选择 ------------------
        ...
        # # NOTE - 利用CCCP求解以计算奖励
        # total_reward, reward, total_delay = self.Reward.reward_compute(L_k1=self.L_k1, L_k2=self.L_k2, tau=self.tau,
        #                                             D_k1=first_trans_period, channel_gain=h_user_2_uav,
        #                                             uav_pos_pre=self.cur_uav_pos, uav_pos=next_uav_pos,
        #                                             choose=uav_selection, noma_group_num=self.noma_group_num)
        total_reward, reward, total_delay = self.Reward.DRL_reward(L_k1=self.L_k1, L_k2=self.L_k2, tau=self.tau,
                                                                   D_k1=first_trans_period, D_k2=second_trans_period,
                                                                   p_k1_D1=p_k1_D1, p_k2_D1=p_k2_D1, p_k2_D2=p_k2_D2,
                                                                   f_uav=f_uav, channel_gain=h_user_2_uav,
                                                                   uav_pos_pre=self.cur_uav_pos, uav_pos=next_uav_pos,
                                                                   choose=uav_selection, noma_group_num=self.noma_group_num)
        ...
        if self.t < self.total_time_slots:
            ...
            next_h_user_2_uav, next_h_user_2_ris, next_h_ris_2_uav = \
                RIS.channel_compute(self.users_num_per_noma_group * self.noma_group_num, self.uav_num,
                                    self.noma_group_num, self.t, self.total_time_slots, self.seed)
            # 拼接以下四个状态成为状态空间:
            # UAV 坐标: self.cur_uav_pos
            # 用户坐标: self.users_pos
            # 用户到 RIS 信道: next_h_user_2_ris
            # RIS 到 UAV 信道: next_h_ris_2_uav
