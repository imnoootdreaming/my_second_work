...
def load_trained_models(matd3, model_dir, device, uav_num):
    """
    加载训练好的MATD3模型权重
    ...
    for uav_idx in range(uav_num):
        ...
        critic_1_path = os.path.join(model_dir, f'uav_{uav_idx}_critic1.pth')
        critic_2_path = os.path.join(model_dir, f'uav_{uav_idx}_critic2.pth')

        if os.path.exists(actor_path):
            matd3.agents[uav_idx].actor.load_state_dict(
        ...
        if os.path.exists(critic_1_path) and os.path.exists(critic_2_path):
            matd3.agents[uav_idx].critic_1.load_state_dict(
                torch.load(critic_1_path, map_location=device)
            )
            matd3.agents[uav_idx].critic_2.load_state_dict(
                torch.load(critic_2_path, map_location=device)
            )
    ...
    ris_critic_1_path = os.path.join(model_dir, 'RIS_critic1.pth')
    ris_critic_2_path = os.path.join(model_dir, 'RIS_critic2.pth')

    if os.path.exists(ris_actor_path):
        matd3.agents[uav_num].actor.load_state_dict(
    ...
    if os.path.exists(ris_critic_1_path) and os.path.exists(ris_critic_2_path):
        matd3.agents[uav_num].critic_1.load_state_dict(
            torch.load(ris_critic_1_path, map_location=device)
        )
        matd3.agents[uav_num].critic_2.load_state_dict(
            torch.load(ris_critic_2_path, map_location=device)
        )
...
def test_episode(env, matd3, running_norms, uav_num, total_time_slots):
    """
    执行一个测试回合
    ...
    with torch.no_grad():  # 测试时不需要梯度
        for t in range(total_time_slots):
            ...
            # 选择动作（不添加探索噪声）
            actions = matd3.take_action(s_norm, explore=False)
...
if __name__ == "__main__":
    ...
    # ========= 模型路径配置 ========= #
    # 请修改为你的模型文件所在目录
    model_dir = "./checkpoint/MATD3_models"  # 修改为你的模型保存路径
    ...
    # ========= 加载训练好的模型 ========= #
    load_trained_models(matd3, model_dir, device, uav_num)
    ...
    for test_ep in tqdm(range(num_test_episodes), desc="测试进度"):
        episode_rewards, agent_rewards, uav_traj, user_traj = test_episode(
            env, matd3, running_norms, uav_num, total_time_slots
        )
    ...
    df_uav.to_csv(f'{today_str}_MATD3_test_uav_traj_seed{seed}.csv', index=False)
    ...
    df_user.to_csv(f'{today_str}_MATD3_test_user_traj_seed{seed}.csv', index=False)
    print("✅ 用户轨迹已按长表格格式保存")
    ...
    df_rewards.to_csv(f'{today_str}_MATD3_test_rewards_seed{seed}.csv', index=False)
    print("✅ 每时隙奖励已保存到CSV")